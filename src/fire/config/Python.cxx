...
static Parameters getMembers(PyObject* object) {
  ...
  if (dictionary == 0) {
    if (PyDict_Check(object))
      dictionary = object;
    else if (PyObject_HasAttrString(object, "__dict__"))
      dictionary = PyObject_GetAttrString(object, "__dict__");
    else
      throw Exception("Python","Python object does not have a __dict__ member",false);
  }
...
Parameters run(const std::string& pythonScript, char* args[], int nargs) {
  ...
  if (fp.get() == NULL) {
    // file does not exist
    throw Exception("Python","Configuration script "+pythonScript+
  ...
  if (PyRun_SimpleFile(fp.get(), pythonScript.c_str()) != 0) {
    ...
    PyErr_Print();
    throw Exception("Python","Execution of python script failed.",false);
  } 
  ...
  if (!py_root_obj) {
    PyErr_Print();
    throw Exception("Python","I don't know what happened. This should never happen.");
  }
  ...
  while (std::getline(root_obj_ss, attr, '.')) {
    ...
    if (one_level_down == 0) {
      throw Exception("Python","Unable to find python object '"+attr+"'.",false);
    }
  ...
  if (py_root_obj == Py_None) {
    ...
    throw Exception("Python","Root configuration object "+root_object+" not defined. This object is required to run.",false);
  }
  ...
  if (Py_FinalizeEx() < 0) {
    PyErr_Print();
    throw Exception("Python","I wasn't able to close up the python interpreter!",false);
  }
